...
function cal_WebCalendar(){ //初始化日历的设置
    ...
    this.callBackFunction   = null;             //回调函数(如果有则回调,如果没有则什么也不做)
...
function calendar(pName, pCallBackFunction){
    if (pCallBackFunction != null) {
        WebCalendar.callBackFunction = pCallBackFunction;
    }
    var e;
    var ev = cal_SearchEvent();
    if(ev)
    ...
    if (arguments.length == 0) {
        ...
        // Ryou add start
        if (e) {
            var vTD = e.parentNode;
            var vEventSrc = vTD.getElementsByTagName('INPUT');
            for (var i = 0; i < vEventSrc.length; i++) {
                if (vEventSrc[i].name == pName) {
                    WebCalendar.objExport = vEventSrc[i];
                    break;
                }
            }
...
function cal_returnDate(){ //根据日期格式等返回用户选定的日期
    if(WebCalendar.objExport)
    {
        ...
        var vYear = a[2];
        var vMonth = cal_appendZero(a[1]);
        var vDay = cal_appendZero(a[0]);
        var returnValue = flag ? vYear +d[2]+ vMonth +d[2]+ vDay : a[2] +d[2]+ a[1] +d[2]+ a[0];
        ...
        if (WebCalendar.callBackFunction != null) {
            WebCalendar.callBackFunction(WebCalendar.objExport, vYear, vMonth, vDay);
        }
        cal_hiddenCalendar();
    }
}
